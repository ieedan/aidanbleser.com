# I love Rust and I could give a f@#k about performance

Many people turn to Rust for performance or security reasons but even if you aren't primary concerned with either Rust is still a great choice.

Rust is a language that pushes the envelope in many different ways (I promise I'm not talking about the borrow checker). Rust brings high level simplicity to low level tasks. For example how might you iterate over every line in a file in JavaScript?
...
If you're a JavaScript developer and you are still upset about the last example then I would like to ask how const works in your language?

Now how about we move on to something we can all agree on, we all hate classes. Now while I do hate classes the utility they provide in name-spacing your functions in quite nice. Rust gives you the ability to namespace things in a functional way with their modules feature.

If you are familiar with a language such as C# you name-spacing is something you are familiar with however with something like C# you are forced to either fully qualify the class or specify the namespace by having a `using` at the top of your file, there is no `partial` qualification.
...
Rust has just prevented a fatal bug before the program could even be built. If you were to have shipped the TypeScript code as soon as you ran into a case where the `phoneNumber` was null you would've got `Cannot read properties of null (reading 'endsWith')` and seen your program crash.
...
For example what I want to determine the type of the IP address and use it accordingly. With enums in Rust this is far more rigid than in other languages.
...
Traits can also be used similarly to interfaces in other languages where methods can require a certain interface to be implemented on their arguments.
...
This isn't unique but what makes this powerful is when you want to enforce multiple trait implementations on a type. This is done with the `+` operator.
...
Error handling is also reconsidered in Rust. Instead of having Exceptions like many other languages Rust offers 2 methods of handling errors.
...
Handle the error with an `if let` or `match` statement:
...
Use Rusts error handling syntax with the `?` to automatically return the `Err` result from the parent function:
...
Those are just some of the syntactical reasons that make writing rust pleasant but what about some of its utilities.

Rust comes out of the box with formatting and linting which enforces preferences not across your project, across the language. This makes code across different Rust projects consistent and prevents arguments about code style preferences.
